import { nest, min, sum, ascending, max, interpolateNumber } from "d3";
...
export default function() {
    ...
    function computeLeafNodes() {
        leafNodes = nodes.filter(node => !node.children.length);
    }

    function computeParentNodes() {
        parentNodes = nodes.filter(node => node.children.length);
    }
    ...
    function visible(linkCollection) {
        return linkCollection.filter(
            link =>
            link.source.state === "expanded" && link.target.state === "expanded"
        );
    }
    ...
    function computeAncestorLinks() {
        ...
        leafNodes.forEach(leafNode => {
            leafNode.sourceLinks.forEach(sourceLink => {
                let ancestorTargets,
                    target = sourceLink.target;
                if (leafNodes.indexOf(target) >= 0) {

                    ancestorTargets = target.ancestors.filter(
                        tAncestor => leafNode.ancestors.indexOf(tAncestor) < 0
                    );
                    ancestorTargets.forEach(ancestorTarget => {
                        const ancestorLink = {
                            source: leafNode,
            ...
            leafNode.targetLinks.forEach(targetLink => {
                let ancestorSources,
                    source = targetLink.source;
                if (leafNodes.indexOf(source) >= 0) {
                    ancestorSources = source.ancestors.filter(
                        sAncestor => leafNode.ancestors.indexOf(sAncestor) < 0
                    );
                    ancestorSources.forEach(ancestorSource => {
                        const ancestorLink = {
        ...
        parentNodes.forEach(parentNode => {
            parentNode.sourceLinks.forEach(sourceLink => {
                let ancestorTargets,
                    target = sourceLink.target;
                if (leafNodes.indexOf(target) >= 0) {
                    ancestorTargets = target.ancestors.filter(
                        tAncestor => parentNode.ancestors.indexOf(tAncestor) < 0
                    );
                    ancestorTargets.forEach(ancestorTarget => {
                        const ancestorLink = {
                            source: parentNode,
    ...
    function mergeLinks() {
        const linkGroups = nest()
            .key(link => link.source.id + "->" + link.target.id)
            .entries(links)
            .map(object => object.values);
        links = linkGroups.map(linkGroup =>
            linkGroup.reduce((previousLink, currentLink) => ({
                source: previousLink.source,
                target: previousLink.target,
                id: min([previousLink.id, currentLink.id]),
    ...
    function nodeHeight(sideLinks) {
        ...
        let spacing = Math.max(sideLinks.length - 1, 0) * linkSpacing,
            scaledValueSum = sum(sideLinks, value) * yScaleFactor;
    ...
    function computeNodeValues() {
        nodes.forEach(node => {
            ...
            node.value = Math.max(
                sum(node.leftLinks, value),
                sum(node.rightLinks, value)
            );

            node.netFlow =
                sum(visible(node.targetLinks), value) -
                sum(visible(node.sourceLinks), value);
            ...
            node.linkSpaceCount = Math.max(
                Math.max(node.leftLinks.length, node.rightLinks.length) - 1,
                0
            );
        });
    }
    ...
    function sourceAndTargetNodesWithSameX() {
        ...
        links.filter(link => link.target.x === link.source.x).forEach(link => {
    ...
    function compressInXDirection() {
        let connectedNodesXPositions,
            nodesByXPosition = nest()
            .key(node => node.x)
            .sortKeys(ascending)
            .entries(nodes)
            .map(object => object.values);

        nodesByXPosition.forEach(xnodes => {
            xnodes.forEach(node => {
                connectedNodesXPositions = node.connectedNodes.map(
                    connectedNode => connectedNode.x
                );
    ...
    function scaleNodeXPositions() {
        ...
        let minX = min(nodes, node => node.x),
            maxX = max(nodes, node => node.x) - minX;
    ...
    function computeNodeYPositions(iterations) {
        ...
        function calculateYScaleFactor() {
            ...
            yScaleFactor = min(nodesByXPosition, nodes => {
                linkSpacesCount = sum(nodes, node => node.linkSpaceCount);
                nodeValueSum = sum(nodes, node => node.value);
                discretionaryY =
                    size[1] -
            ...
            links.forEach(link => {
                let linkLength = Math.abs(link.source.x - link.target.x),
        ...
        function initializeNodeYPosition() {
            nodesByXPosition.forEach(nodes => {
                nodes.forEach((node, i) => {
                    node.y = i;

                    node.heightAllowance =
                        node.value * yScaleFactor + linkSpacing * node.linkSpaceCount;
                });
            });
        }
            ...
            nodesByXPosition.forEach(nodes => {
                nodes.forEach(node => {
                    ...
                    if (node.rightLinks.length) {
                        const y =
                            sum(node.rightLinks, weightedSource) /
                            sum(node.rightLinks, value);
                ...
                .forEach(nodes => {
                    nodes.forEach(node => {
                        if (node.leftLinks.length) {
                            const y =
                                sum(node.leftLinks, weightedTarget) /
                                sum(node.leftLinks, value);
                            node.y += (y - center(node)) * alpha;
                        }
                    });
    ...
    function computeLinkYPositions() {
        function ascendingLeftNodeYPosition(a, b) {
            ...
            let aLeftNode = a.direction > 0 ? a.source : a.target,
        ...
        function ascendingRightNodeYPosition(a, b) {
            ...
            let aRightNode = a.direction > 0 ? a.target : a.source,
    ...
    biHiSankey.collapsedNodes = function() {
        return nodes.filter(node => node.state === "collapsed");
    };
    ...
    biHiSankey.expandedNodes = function() {
        return nodes.filter(node => node.state === "expanded");
    };
    ...
    biHiSankey.link = function() {
        let curvature = defaultLinkCurvature;


        function leftToRightLink(link) {
            let arrowHeadLength = link.thickness * arrowheadScaleFactor,
                ...
                xi = interpolateNumber(x0, x4),
                x2 = xi(curvature),
                x3 = xi(1 - curvature),
                y0 = link.source.y + link.sourceY + link.thickness / 2,
                y1 = link.target.y + link.targetY + link.thickness / 2;
            return `M${x0},${y0}L${x1},${y0}C${x2},${y0} ${x3},${y1} ${x4},${y1}L${x4 +
        straightSectionLength},${y1}`;
        }
